...
{
    /// <summary>
    /// Implements a fully editable byte provider for file regions of any size.
    /// </summary>
    /// <remarks>
    /// Only changes to the file are stored in memory with reads from the
    /// original regions occurring as required.
    /// </remarks>
    public sealed class DynamicFileByteProvider : IByteProvider, IDisposable
    {
        ...
        /// Constructs a new <see cref="DynamicFileByteProvider" /> instance.
        /// </summary>
        /// <param name="stream">the stream containing the regions.</param>
        /// <remarks>
        ...
        {
            if (_readOnly)
                throw new OperationCanceledException("FileWorker_ReadCompleted is in read-only mode");
